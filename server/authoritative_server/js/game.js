...
function create() {
  ...
  io.on('connection', function (socket) {
    ...
    players[socket.id] = {
      ...
      playerId: socket.id,
      team: (playersDetect[0] === players[socket.id]) ? 'red' : 'blue',
      input: {
        left: false,
        right: false,
        up: false
    ...
    if (playersDetect.length == 2 || playersDetect.length > 2 ) {
    ...
    socket.on('textureKeyPrivate', checkTexturePrivate);
      ...
      
{
    ...
    socket.on('cardPlayedPrivate', function (gameObject, isPlayerA) {
      //console.log ('cardPlayed');
      socket.broadcast.emit('cardPlayedPrivate', gameObject, isPlayerA);
    });
